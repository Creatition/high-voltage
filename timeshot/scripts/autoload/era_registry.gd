...
func _register_all() -> void:
	...
	_add("prehistoric",  "Prehistoric",        "Charging dinos and bleed traps.",                    Color(0.65, 0.85, 0.35),  _prehistoric_rooms(),             true,  false)
	...
	_add("medieval",     "Medieval",           "Plate, pikes, and a stubborn baron.",                Color(0.78, 0.70, 0.55),  _medieval_rooms(),                true,  false)
	...
	_add("future",       "Future",             "Shielded enemies and EMP rooms.",                    Color(0.55, 0.95, 0.85),  _future_rooms(),                  true,  false)
...
func _medieval_rooms() -> Array:
	# Medieval scenes are built in Day 11; same fallback strategy.
	var first := "res://scenes/rooms/medieval_room_01.tscn"
	if ResourceLoader.exists(first):
		return [
			first,
...
func _prehistoric_rooms() -> Array:
	var first := "res://scenes/rooms/prehistoric_room_01.tscn"
	if ResourceLoader.exists(first):
		return [
			first,
...
func _future_rooms() -> Array:
	# "Future" era → cyberpunk theme. Day 13.
	var first := "res://scenes/rooms/cyberpunk_room_01.tscn"
	if ResourceLoader.exists(first):
		return [
			first,
